...
func parseIntsSpaced(s string) int64 {
	var spacelessInt string = ""
	var ret int64 = 0
	for _, stringValue := range strings.Fields(s) {
		spacelessInt += stringValue
	}
	value, err := strconv.Atoi(spacelessInt)
	if err != nil {
		fmt.Printf("Error converting string to int: %v\n", err)
		return ret
	}
...
func (r *Runner) parse() {
	...
	time := parseIntsSpaced(strings.Split(lines[0], ": ")[1])
	record := parseIntsSpaced(strings.Split(lines[1], ": ")[1])
	r.part2race = RaceInfo{time, record}
}
