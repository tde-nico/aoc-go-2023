...
func (m *Mapping) reverse_lookup(val int64) int64 {
	for _, mp := range m.maps {
		rev := val - mp.delta
		if mp.rng.contains(rev) {
			return rev
		}
	}
	return val
...
func (r *Runner) part2() {
	seed_ranges := make([]Range, 0)
	for i := 0; i < len(r.seeds); i += 2 {
		rng_val := r.seeds[i : i+2]
		seed_ranges = append(seed_ranges, Range{
			start: rng_val[0],
			end:   rng_val[0] + rng_val[1],
		})
	}
	var location int64 = 1
	for {
		cur := location
		for i := len(r.mapping) - 1; i >= 0; i-- {
			cur = r.mapping[i].reverse_lookup(cur)
		}
...
func (r *Runner) part2slow() {
	var mn int64 = math.MaxInt64
	for i := 0; i < len(r.seeds); i += 2 {
		seed_range := r.seeds[i : i+2]
		for seed := seed_range[0]; seed <= seed_range[0]+seed_range[1]; seed++ {
			cur := seed
			for _, mp := range r.mapping {
				cur = mp.apply_map(cur)
			}
